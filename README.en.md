...
> Current version: **v26.5.68** (2026-05-28) · German version: [README.md](README.md)
...
```bash
# one-time clone (into current path; directory should be empty)
...
Grab the latest release asset (`plantuml-renderer.html`) from [github.com/jcmx9/plantuml-renderer/releases/latest](https://github.com/jcmx9/plantuml-renderer/releases/latest) — no other files needed.
...
| `[A] requires 5 days then` (with follow-up line) | Multi-line succession: trailing `then`, next line is the successor. |
...
| `[B] displays on same row as [A]` | Task/milestone B shares the row with A (no own label, own bar). |
